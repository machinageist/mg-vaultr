...
- **One YAML adapter in the workspace, not two.** `crates/mg-vault-core/src/frontmatter_scalar.rs` is generalized behind that adapter. I owns the read/write model (`yaml/{model,index,edit,style,comments,verify}.rs`) and the CST index. Two YAML dialects in this workspace is a defect, not a design choice.
